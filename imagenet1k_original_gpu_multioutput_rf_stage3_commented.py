#!/usr/bin/env python3
"""
ImageNet-1K ORIGINAL
CUSTOM GPU SHARED-TREE MULTI-OUTPUT RANDOM FOREST ECOC / N-ARY ECOC — STAGE 3
================================================================================

PURPOSE
-------
This script contains the CUSTOM GPU SHARED-TREE RANDOM FOREST implementation
used for the Multi-output RF-ECOC experiments in the dissertation.

IMPORTANT IMPLEMENTATION NOTE
-----------------------------
The Multi-output RF-ECOC model in this file is NOT implemented with:

    - sklearn.ensemble.RandomForestClassifier
    - sklearn.multioutput.MultiOutputClassifier
    - cuml.ensemble.RandomForestClassifier

scikit-learn is used only for utility functions such as train/validation
splitting and evaluation metrics. The Random Forest training itself is
implemented directly in this file using CuPy and custom CUDA kernels.

WHY A CUSTOM GPU SHARED-TREE RF IS USED
---------------------------------------
The required Multi-output RF-ECOC architecture is different from Independent
RF-ECOC.

Independent RF-ECOC:
    - trains one independent Random Forest per ECOC code position;
    - with code length L, this requires L separate forests;
    - every ECOC position therefore has its own tree structures.

Multi-output RF-ECOC in this script:
    - trains ONE Random Forest for each (seed, N, L) configuration;
    - every tree has ONE feature/threshold structure shared by ALL L ECOC
      output positions;
    - every terminal leaf stores an L-dimensional predicted ECOC symbol vector.

The custom implementation is required because:
    - cuML provides GPU Random Forest training, but does not provide the native
      shared multi-output classification structure required here;
    - scikit-learn supports native multi-output Random Forest classification,
      but its Random Forest implementation is CPU-based;
    - this Stage-3 implementation provides BOTH shared multi-output tree
      structure AND GPU execution.

In short, the objective is:

    ONE shared forest + L ECOC outputs + GPU execution

HOW THE CUSTOM SHARED-TREE RF WORKS
-----------------------------------
1. The ECOC codebook is generated for the requested alphabet size N and code
   length L.
2. The codebook is converted into a class-agreement matrix A.  A[c,c'] is the
   fraction of ECOC positions in which classes c and c' share the same symbol.
3. Each decision-tree node samples candidate features, builds GPU histograms,
   and scores candidate thresholds.
4. Splits are selected using the agreement-matrix formulation of mean
   multi-output Gini impurity:

       G(q) = 1 - (q^T A q) / n^2

   where q is the class-count vector in the node and n = sum(q).
5. ONE feature and ONE threshold are selected for the node.  That split is
   therefore shared by all L ECOC output positions.
6. At each terminal leaf, class counts are converted through the ECOC codebook
   into per-position symbol counts, producing one L-dimensional leaf symbol
   vector.
7. Every tree predicts one L-dimensional vector.  Forest voting is performed
   independently for every ECOC position.
8. The final L-dimensional forest output is decoded to the original class using
   validation-derived Weighted Hamming decoding.

IMPORTANT: the trainer receives the ORIGINAL CLASS LABELS, not a B x L target
matrix.  The shared multi-output ECOC objective is represented through the
ECOC codebook and its derived class-agreement matrix.

WHERE THE CUSTOM RF IS IMPLEMENTED IN THIS FILE
-----------------------------------------------
The main custom components are:

    _HISTOGRAM_KERNEL
        Custom CuPy RawKernel that builds class histograms for all selected
        candidate features at a tree node.

    _TREE_PREDICT_KERNEL
        Custom CuPy RawKernel that traverses one trained shared tree.

    _VOTE_KERNEL
        Custom CuPy RawKernel that accumulates per-position symbol votes across
        trees.

    build_codebook_embedding_and_agreement()
        Builds the one-hot codebook embedding H and class-agreement matrix A.

    Stage3SharedTreeTrainer
        The core custom decision-tree trainer.  It chooses shared node splits
        using the agreement-matrix multi-output Gini criterion.

    FlatSharedTree
        GPU-friendly representation of one trained shared tree.  Each leaf
        stores an L-dimensional symbol vector.

    run_one_configuration()
        Trains ONE forest for one (seed, N, L) configuration, performs
        per-position forest voting, derives decoder weights from validation
        data, decodes validation/test predictions, and saves result.csv.

BACKEND IDENTIFIER
------------------
All configuration/result files identify this implementation as:

    custom_gpu_stage3_shared_tree

This makes it explicit that the reported Multi-output RF-ECOC results come from
this custom backend rather than from standard cuML or scikit-learn RF.

DATA / EXPERIMENT PROTOCOL
--------------------------
Frozen DINOv3 768-D features are used without standardisation.

Default seeds:
    123, 231, 340, 451, 562

Default alphabet sizes:
    N = 2, 3, 5, 8, 16
...
Default Random Forest settings:
    n_estimators       = 300
    criterion          = exact mean multi-output Gini via class agreement
    max_depth          = None / unrestricted   (CLI: --max-depth 0)
    min_samples_leaf   = 1
    max_features       = sqrt
    histogram bins     = 128
    bootstrap          = True
...
Data protocol:
    90% of official ImageNet-1K train cache -> RF training
    10% of official ImageNet-1K train cache -> internal validation
    official ImageNet validation cache      -> fixed final test

Decoder:
    validation-derived Weighted Hamming
    (the fixed test set is NEVER used to derive decoder weights)

HOW TO RUN THIS CUSTOM GPU SHARED-TREE RF
-----------------------------------------
Activate the environment first, for example:

    micromamba activate /home/am2618/micromamba/envs/food101-cuml-rf-2608

A) SMALL SMOKE TEST (debug only)
--------------------------------
Use one seed, one N value, one code length, one tree and a small training subset:

    python imagenet1k_original_gpu_multioutput_rf_stage3_commented.py \
        --seeds 123 \
        --n-values 2 \
        --code-lengths 64 \
        --n-estimators 1 \
...
Any run with --max-train-samples > 0 is DEBUG ONLY and must not be reported as
final ImageNet-1K dissertation results.

B) ONE FULL CONFIGURATION
-------------------------
Example: seed 123, binary ECOC, L=64, 300 trees:

    CUDA_VISIBLE_DEVICES=0 python imagenet1k_original_gpu_multioutput_rf_stage3_commented.py \
        --seeds 123 \
        --n-values 2 \
        --code-lengths 64 \
        --n-estimators 300 \
...
C) FULL FINAL GRID
------------------
The defaults already contain the five seeds, five N values and four code
lengths.  Therefore a full run can be started with:
...
For final thesis results, --max-train-samples MUST remain 0.

CHECKPOINT / RESUME
-------------------
Vote tensors and the completed-tree count can be checkpointed during a
configuration.  Completed result.csv configurations are automatically skipped
when their settings match the requested configuration.  Therefore the same
command can be restarted after interruption and matching checkpoints/results
will be reused where possible.

OUTPUTS
-------
Each completed (seed, N, L) configuration stores files such as:

    config.json
    result.csv
    ecoc_artifacts.npz

During long runs, checkpoint files may also be written:

    checkpoint_meta.json
    checkpoint_val_votes.npy
    checkpoint_test_votes.npy

The script also rebuilds combined detailed/summary/report CSV files from all
completed result.csv files.

FULL RUN WARNING
----------------
This file is a complete experiment runner, but the custom RF is computationally
expensive on full ImageNet-1K.  Benchmark a 1-tree and/or 10-tree run before
launching the complete grid.
"""
